"""Train MOPO with full-history TPNN rollouts and RPNN evaluation."""
...
from offlinerlkit.policy.model_based.mopo_tpnn import (  # noqa: E402
    TPNNMOPOPolicy,
)
...
def get_args():
    parser = argparse.ArgumentParser(
        description=(
            "5-member MOPO with Ensemble TPNN full-history training "
            "rollouts and "
            "25-member RPNN evaluation"
        )
    )
    parser.add_argument(
        "--algo-name",
        type=str,
        default="mopo_tpnn_ensemble5_full_history",
    )
    ...
    parser.add_argument(
        ...
        default=[0, 1, 2, 3, 4],
        help=(
            "Exactly five source TPNN members used for trajectory "
...
def _algorithm_config(args) -> dict:
    ...
    return {
        ...
        "training_member_indices": list(args.training_member_indices),
...
def _canonical_config(args, dynamics_model, evaluation_metadata) -> dict:
    return {
        ...
        "dynamics_config": {
            "training": {
                "architecture": "TPNN",
                "ensemble_members": len(args.training_member_indices),
                "source_ensemble_members": dynamics_model.num_ensemble,
                "member_indices": list(args.training_member_indices),
...
def train(args=None):
    ...
    training_member_indices = getattr(
        args, "training_member_indices", None
    )
    if training_member_indices is None:
        training_member_indices = [0, 1, 2, 3, 4]
    args.training_member_indices = [
        int(member_idx) for member_idx in training_member_indices
    ]
    if (
        len(args.training_member_indices) != 5
        or len(set(args.training_member_indices)) != 5
    ):
        raise ValueError(
            "5-member MOPO requires exactly five unique "
            "training_member_indices."
        )
    if float(args.penalty_coef) < 0:
    ...
    args.training_dynamics_members = dynamics.num_training_members
    args.training_member_indices = (
    ...
    dynamics_model.save_member_manifest(
        args.tpnn_member_manifest_path,
        member_indices=args.training_member_indices,
    )
    _write_json(
    ...
    print(
        ...
        f"({dynamics.num_training_members} active members "
        f"{dynamics.training_member_indices.tolist()} from "
        f"{dynamics_model.num_ensemble} source members; "
    ...
    print(
        f"MOPO uncertainty={args.uncertainty_mode}, "
        f"penalty coefficient={args.penalty_coef}"
    )
    print(
